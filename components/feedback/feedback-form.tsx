...
import { Star, CheckCircle2, Send, CalendarDays } from 'lucide-react'
...
/**
 * Context resolved from a session QR. When present the participant rates a
 * known training: the fields are shown read-only and the server re-reads them
 * from the session row, so nothing here can be tampered with or mistyped.
 */
...
function SessionHeader({ session }: { session: SessionContext }) {
  const meta = [session.trainer, session.organization, session.location].filter(Boolean).join(' · ')
  return (
    <Card className="border-primary/30 bg-primary/5">
      <CardContent className="py-5">
        <p className="text-xs font-semibold uppercase tracking-wider text-primary">You&apos;re rating</p>
        <p className="mt-1.5 text-lg font-semibold leading-snug text-foreground">{session.trainingName}</p>
        {session.trainingTopic && (
          <p className="mt-0.5 text-sm text-muted-foreground">{session.trainingTopic}</p>
        )}
        {(meta || session.trainingDate) && (
          <p className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-muted-foreground">
            {meta}
            {meta && session.trainingDate && <span aria-hidden="true">·</span>}
            {session.trainingDate && (
              <span className="inline-flex items-center gap-1">
                <CalendarDays className="h-3.5 w-3.5" /> {session.trainingDate}
              </span>
            )}
          </p>
        )}
...
export function FeedbackForm({ session }: { session?: SessionContext }) {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    setErr('')
    if (!session && !text.trainingName.trim()) { setErr('Please enter the training name.'); return }
    if (!ratings.overall) { setErr('Please give an overall rating.'); return }
    setStatus('submitting')
    try {
      // With a session, context comes from the server — send only the answers.
      const payload = session
        ? {
            sessionCode: session.code,
            likedMost: text.likedMost, improve: text.improve, suggestions: text.suggestions,
            website: text.website, ...ratings, nps: nps ?? undefined,
          }
        : { ...text, ...ratings, nps: nps ?? undefined }
